...
export default function CompanyUpgradeScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
        ...
        <View style={styles.content}>
              ...
              <TouchableOpacity
                ...
                style={[
                  styles.planCard,
                  {
                    ...
                    borderWidth: 2,
                    backgroundColor: plan.highlighted ? colors.primary + '10' : colors.card,
                  },
                ]}
              >
                {plan.highlighted && (
                ...
                <View style={styles.priceSection}>
                  ...
                  <Text style={[styles.pricePerMonth, { color: colors.mutedForeground }]}>/month</Text>
                </View>
                <View style={styles.featuresList}>
                  {plan.features.map((feature, index) => (
                    <View key={index} style={styles.featureItem}>
                      <Ionicons name="checkmark-circle" size={18} color={colors.primary} />
                      <Text style={[styles.featureText, { color: colors.foreground, marginLeft: 8 }]}>
                        {feature}
                      </Text>
                    </View>
                  ))}
          ...
          <Card style={styles.faqCard}>
            ...
            <View style={styles.faqItem}>
              <Text style={[styles.faqQuestion, { color: colors.foreground }]}>Can I cancel anytime?</Text>
              <Text style={[styles.faqAnswer, { color: colors.mutedForeground }]}>
                Yes, you can cancel your subscription at any time. No questions asked.
              </Text>
            </View>
            <View style={styles.faqItem}>
              <Text style={[styles.faqQuestion, { color: colors.foreground }]}>Is there a free trial?</Text>
              <Text style={[styles.faqAnswer, { color: colors.mutedForeground }]}>
          ...
          {/* Action Buttons */}
          {isPremium ? (
            <Button onPress={() => router.back()} style={[styles.button, { backgroundColor: colors.primary }]}>
            ...
            <View style={styles.actionButtons}>
              ...
              >
                ...
                <Text style={{ color: '#fff', fontWeight: '600', marginLeft: 8 }}>Upgrade Now</Text>
              </Button>
            </View>
          )}
        </View>
      ...
      <MockPaymentSheet
        visible={showPayment}
        amount={selectedPlanObj?.price ?? '£79'}
        description={`Svype Premium – ${selectedPlanObj?.name ?? 'Professional'}`}
...
const styles = StyleSheet.create({
  ...
  benefitsBanner: { padding: 16, flexDirection: 'row', alignItems: 'center' },
  benefitsContent: { flex: 1, flexDirection: 'row', alignItems: 'center' },
  benefitsTitle: { fontSize: 14, fontWeight: '600', marginBottom: 4 },
  benefitsText: { fontSize: 12 },
  plansContainer: { gap: 12 },
  planCard: { borderRadius: 12, padding: 16, position: 'relative' },
  popularBadge: {
    position: 'absolute',
    top: -8,
    left: 16,
  ...
  planDescription: { fontSize: 12, marginBottom: 12 },
  priceSection: { flexDirection: 'row', alignItems: 'baseline', marginBottom: 16 },
  planPrice: { fontSize: 32, fontWeight: '700' },
  pricePerMonth: { fontSize: 12, marginLeft: 4 },
  featuresList: { gap: 8 },
